...
bool Pass2::process(const std::vector<std::string>& sourceLines) {
    ...
    for (const auto& line : sourceLines) {
        ...
        // Simplified parsing (same as Pass 1)
        ...
        if (opcode == "LTORG") continue; // Handled in pass 1
        ...
        if (opcode == "BYTE") {
            ...
            if (inst.format == InstructionFormat::FORMAT_4 && !label.empty()) {
                objWriter.addModificationRecord(loc + 1, 5); // 5 half-bytes = 20 bits
            }
        }
...
std::vector<Byte> Pass2::generateObjectCode(const Instruction& inst) {
    ...
    switch (inst.format) {
        case InstructionFormat::FORMAT_1:
            return genFormat1(opcode);
        case InstructionFormat::FORMAT_2:
            return genFormat2(opcode, inst.operand);
        case InstructionFormat::FORMAT_3:
            return genFormat3(opcode, inst.operand, inst.location, Flags());
        case InstructionFormat::FORMAT_4:
            return genFormat4(opcode, inst.operand, inst.location, Flags());
        default:
            return {};
    }
}
...
std::vector<Byte> Pass2::genFormat3(uint8_t op, const std::string& operand, 
    ...
    if (isImmediateValue && targetAddr <= 4095) {
        // Immediate value fits in 12 bits
        displacement = targetAddr;
        flags.n = 0; flags.p = 0; flags.b = 0;
    } else {
        // Try PC-relative
        int pc = loc + 3;
        int disp = targetAddr - pc;
        if (disp >= -2048 && disp <= 2047) {
            displacement = disp & 0xFFF;
            flags.p = 1;
        } else if (baseSet) {
            // Try Base-relative
            disp = targetAddr - baseRegister;
            if (disp >= 0 && disp <= 4095) {
                displacement = disp;
                flags.b = 1;
            } else {
